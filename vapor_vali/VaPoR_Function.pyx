...
def bp_let_to_hash(info,flank_length):
	...
	out_hash_keys=list(out_hash.keys())
...
def cluter_to_diagnal(out,clu_dis_cff,lenght_cff,dots_num_cff):
	...
	for k1 in range(len(out2[1])):
		if not out2[1][k1] in list(out2_hash1.keys()):
			out2_hash1[out2[1][k1]]=[]
		out2_hash1[out2[1][k1]].append(out2[0][k1])
	cluster_a=cluster_numbers(list(out2_hash1.keys()),clu_dis_cff)
	cluster_b=[]
	for x in cluster_a:
		cluster_b.append([])
		for y in x:
...
def list_to_hash(out):
	...
	for x in range(len(out[0])):
		if not out[0][x] in list(out_hash.keys()):
			out_hash[out[0][x]]=[]
...
def qc_ref_file_initiate(qc_file_name):
	if not os.path.isfile(qc_file_name):
		...
		print(' '.join(['sample_name','chromosome','bp1','bp2','diagnal_per','rep_per']), file=fo)
		fo.close()
def qc_ref_file_write(qc_file_name,sample_name,bps,region_QC,affix,seq2):
	if affix=='':
		fo=open(qc_file_name,'a')
		print(' '.join([str(i) for i in [sample_name]+bps+[region_QC[0],max(region_QC[1])/float(len(seq2))]]), file=fo)
		fo.close()
	else:
		fo=open(qc_file_name,'a')
		print(' '.join([str(i) for i in [sample_name]+bps+[region_QC[0],max(region_QC[1])/float(len(seq2))]+[affix]]), file=fo)
		fo.close()
def quality(hits):
...
def qual_check_R_3(dotdata_qual_check):
	...
	for x in range(len(dotdata_qual_check[0])):
		print(' '.join([str(i) for i in [dotdata_qual_check[0][x],dotdata_qual_check[1][x]]]), file=fo)
	fo.close()
...
def read_hash_minimize(all_reads):
	...
	if len(read_hash)>30:
		new_read_hash=random.sample(list(range(len(read_hash))),30)
...
def write_null_individual_stat(txt_file):
	fout='.'.join(txt_file.split('.')[:-1])+'.vapor'
	fo=open(fout,'w')
	print(' '.join(['alt','ref']), file=fo)
	fo.close()
...
def heatmap_data_produce_vertical(matrix_size,dotdata_ref_2,summit,dup_module):
	...
	diagnal_dots=list(range(diagnal_max))
...
def heatmap_matrix_produce_vertical(dotdata_ref_2,summit):
	...
	for k1 in dotdata_ref_2:
		if not k1[0] in list(out.keys()):
			out[k1[0]]={}
		for k2 in range(-summit,summit):
			if not k1[1]-k2 in list(out[k1[0]].keys()):
...
def heatmap_data_produce_round(dotdata_ref_2,summit):
	...
	for x in dotdata_ref_2:
		if not x[0] in list(matrix.keys()):
			matrix[x[0]]={}
		if not x[1] in list(matrix[x[0]].keys()):
			matrix[x[0]][x[1]]=0
		...
		for y in matrix_add_unique_list:
			if not y[1] in list(matrix.keys()):
				matrix[y[1]]={}
			if not y[2] in list(matrix[y[1]].keys()):
				matrix[y[1]][y[2]]=0
			matrix[y[1]][y[2]]+=summit-y[0]
	matrix_size=[list(range(min(matrix_dots[0])-summit,max(matrix_dots[0])+summit)),list(range(min(matrix_dots[1])-summit,max(matrix_dots[1])+summit))]
	diagnal_max=min([max(matrix_size[0]),max(matrix_size[1])])
	diagnal_dots=list(range(diagnal_max))
	diagnal_matrix={}
	...
	for x in diagnal_dots:
		if not x in list(matrix.keys()):
			matrix[x]={}
		if not x in list(matrix[x].keys()):
			matrix[x][x]=0
		matrix[x][x]-=summit
def merge_matrx(matrix_list):
	out={}
	for matrix_1 in matrix_list:
		for k1 in list(matrix_1.keys()):
			for k2 in list(matrix_1[k1].keys()):
				print((k1,k2))
				if not (k1,k2) in list(out.keys()):
					out[(k1,k2)]=matrix_1[k1][k2]
				else:
					out[(k1,k2)]+=matrix_1[k1][k2]
	return out
def print_dot_hash(matrix_dots_hash):
	for k1 in list(matrix_dots_hash.keys()):
		for k2 in list(matrix_dots_hash[k1].keys()):
			print(' '.join([str(i) for i in [k1,k2,matrix_dots_hash[k1][k2]]]))
summit=20
def symmetric_by_diagnal_dots_produce(dot_matrix):
	out={}
	for k1 in list(dot_matrix.keys()):
		for k2 in list(dot_matrix[k1].keys()):
			if not k2 in list(out.keys()):
				out[k2]={}
			if not k1 in list(out[k2].keys()):
...
def write_pacval_individual_stat_simple_long(PacVal_score_hash,txt_file,rsquare_ref1,rsquare_ref2,rsquare_alt,SV_index):
	fo=open('.'.join(txt_file.split('.')[:-1])+'.vapor','w')
	print(' '.join(['alt','ref']), file=fo)
	...
	if len(rsquare_ref1)>0:
		...
		for x in range(len(rsquare_out_ref)):
			print(' '.join([str(i) for i in [rsquare_out_alt[x],rsquare_out_ref[x]]]), file=fo)
	...
	if len(rsquare_ref)>0:
		...
		for x in range(len(rsquare_out_ref)):
			...
			if y[0]>y[1]: 
				qual_score+=1
			total_reads+=1
			print(' '.join([str(i) for i in y]), file=fo)
		if total_reads>0:
			PacVal_score_hash[SV_index]=float(qual_score)/float(total_reads)
	fo.close()
	return PacVal_score_hash
...
def pick_quality_score(ref_list,alt_list):
	...
	total_len_list=list(range(len(alt_list[0])))
			...
			if not read_hash==[]:
				...
				if not window_size=='Error':
					if region_QC[0]>region_QC_Cff or sum(region_QC[1])/float(len(seq2))<0.3:
						...
						fo=open(out_file_Cannot_Validate,'a')
						print(' '.join([str(i) for i in info_new]), file=fo)
						fo.close()
				else:
					window_size=10
		else:
...
def write_deviation_data(fileout,deviation_lists,colnames):
	fo=open(fileout,'w')
	print(' '.join([str(i) for i in colnames]), file=fo)
	...
	for x in range(len(deviation_new[-1])):
		print(' '.join([str(i) for i in [y[x] for y in deviation_new]]), file=fo)
	fo.close()
